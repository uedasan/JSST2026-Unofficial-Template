...
LuaLaTeX / pdfLaTeX でビルドすることもできます:

```powershell
latexmk -lualatex JSST2026_Template_modern.tex
latexmk -pdf JSST2026_Template_modern.tex
```

サポート対象は upLaTeX, LuaLaTeX, pdfLaTeX です。upLaTeX は `dvipdfmx` で PDF 化します。pLaTeX でもビルドできるようにしていますが、明示的なサポート対象ではありません。
